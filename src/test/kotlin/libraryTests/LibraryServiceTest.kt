...
class LibraryServiceTest {
    ...
    private val bookList = arrayListOf(
        ...
        Book("Harry Potter and the Deathly Hallows", arrayListOf(authorList[0]), Year.parse("2007"), Genre.NOVEL, Status.Restoration),
    ...
    fun test_get_all_book_statuses() {
        Assert.assertEquals(bookList.associateWith { it.status }, libraryService.getAllBookStatuses())
    }
    ...
    fun test_register_user() {
        ...
        try {
            libraryService.registerUser(testUser.firstName, testUser.lastName, testUser.birthDate)
        } catch (e: Exception) {
    ...
    fun test_unregister_user_who_has_books() {
        ...
        try {
            ...
            Assert.assertEquals("User has books to return:\nHarry Potter and the Prisoner of Azkaban, Joanne Rowling (author), 1999, NOVEL, Used by Matvey Ivanov (user) 25.3.2001", e.message)
        }
    }
    ...
    fun test_take_book() {
        ...
        Assert.assertEquals(userList[0], libraryService.getBookStatus(bookList[2]).bookUser)
    }
    ...
    fun test_return_book() {
        libraryService.returnBook(bookList[1])
        Assert.assertEquals(Status.Available, libraryService.getBookStatus(bookList[1]))
    }
